    ...
    React.useCallback(() => {
      async function loadGameSettings() {
        try {
          ...
          // Load high score for this mode (skip practice mode)
          if (mode !== 'practice') {
            const savedHighScore = await getHighScore(mode);
      ...
      // Only load if game hasn't started yet
      if (!gameStarted) {
        loadGameSettings();
      }
    }, [gameStarted, mode])
        ...
        >
          ...
          <View style={styles.topRow}>
            ...
            <View style={styles.scoreContainer}>
              ...
              {highScore > 0 && (
                <View style={[styles.scoreItem, { backgroundColor: 'rgba(255,215,0,0.2)' }]}>
                  <Ionicons name="trophy" size={18} color="#FFD700" />
                  <Text style={[styles.scoreText, { fontSize: 14 }]}>{highScore}</Text>
                </View>
              )}
            </View>
          </View>
